...
export default function RaporPage() {
  ...
  return (
    <div className="p-3 sm:p-6 md:p-8 w-full max-w-[min(100%,1680px)]">
      {/* Header */}
      ...
      <div className="border border-border rounded-xl overflow-hidden bg-card shadow-sm">
        <div className="px-4 py-3 border-b border-border flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-muted-foreground hidden sm:inline">Yatay kaydır · tüm kolonlar</span>
          </div>
        </div>
              ...
              <tr className="border-b border-border bg-muted/30">
                {[
                  ...
                  <th
                    key={h}
                    className={cn(
                      "px-3 py-2.5 text-left text-[11px] font-medium text-muted-foreground uppercase tracking-wide whitespace-nowrap",
                      i === 0 && "sticky left-0 z-10 bg-muted/95 backdrop-blur-sm shadow-[2px_0_6px_-2px_rgba(0,0,0,0.08)]"
                    )}
                  >
                    {h}
                  </th>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id} className="border-b border-border/60 hover:bg-accent/30 transition-colors">
                  <td className="px-3 py-3 sticky left-0 z-[1] bg-card shadow-[2px_0_6px_-2px_rgba(0,0,0,0.06)] group-hover:bg-accent/30">
                    <p className="font-medium text-foreground whitespace-nowrap">{row.name}</p>
                    <p className="text-muted-foreground text-xs max-w-[10rem] truncate">{row.role}</p>
                  </td>
