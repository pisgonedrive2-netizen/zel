...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!reason) {
    return NextResponse.json({ error: "Red sebebi (reason) zorunlu." }, { status: 400 });
  }
  ...
  try {
    ...
    if (reqRow.status === "approved") {
      return NextResponse.json({ error: "Onaylanmış başvuru reddedilemez." }, { status: 409 });
    }
    if (reqRow.status === "rejected") {
      return NextResponse.json({ error: "Bu başvuru zaten reddedilmiş." }, { status: 409 });
    }
    ...
    try {
      await getSupabaseAdmin().from("audit_logs").insert({
        ...
        action: "streamer_registration_rejected",
        detail: `${reqRow.displayName} (req=${reqRow.id}) — sebep: ${reason}`,
      });
    } catch {
      /* audit opsiyonel */
    }
