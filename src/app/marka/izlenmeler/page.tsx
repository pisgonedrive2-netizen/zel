    ...
    return (
      <Card className="max-w-lg mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            Hesabınıza marka bağlı değil. Yönetici <Link href="/kullanicilar" className="text-primary underline">Kullanıcılar</Link> sayfasından{" "}
            <strong>Marka</strong> rolü ve marka seçimi atamalıdır.
          </CardDescription>
        </CardHeader>
      </Card>
    );
  ...
  return (
    <div className="mx-auto max-w-[1200px] space-y-6 pb-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Bu markaya ait linkler, yayıncıların girdiği aylık izlenmeler ve haftalık içerik linkleri — salt okunur.
          </p>
        </div>
      ...
      <div className="sticky top-0 z-20 -mx-1 flex items-center justify-center gap-2 rounded-lg border border-border/60 bg-background/95 py-2.5 backdrop-blur-sm">
        <Button variant="ghost" size="sm" onClick={() => navMonth(-1)} className="h-8 w-8 p-0" type="button" title="Önceki ay">
          <ChevronLeft size={14} />
        </Button>
        <div className="min-w-[140px] rounded-md border border-border bg-card px-3 py-1.5 text-center text-sm font-medium capitalize">
          {monthLabel(month)}
        </div>
        <Button variant="ghost" size="sm" onClick={() => navMonth(1)} className="h-8 w-8 p-0" type="button" title="Sonraki ay">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {hasTarget && targetPct !== null && (
          <Card className="md:col-span-2 border-blue-200/60 bg-blue-50/20 dark:border-blue-500/40 dark:bg-blue-950/30">
            <CardHeader className="pb-2">
              ...
              <CardDescription>
                {monthLabel(month)} · hedef {fmtViews(brand.monthlyTarget!)} · link toplamı {fmtViews(linkViewsForMonth)} ({targetPct.toFixed(0)}%)
              </CardDescription>
            </CardHeader>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base">{monthLabel(month)} özet</CardTitle>
              ...
              <ul className="space-y-2">
                {viewRows.map((v) => (
                  <li key={v.id} className="rounded-lg border border-border px-3 py-2 text-sm">
                    ...
                    {v.url && (
                      <a href={v.url} target="_blank" rel="noopener" className="text-[11px] text-blue-600 break-all">
                        {v.url}
                      </a>
                    )}
