...
/**
 * Toplam Kasa KPI: işletme kasası gösterim bakiyesi toplamı.
 * Ramiz TRON cüzdanı hariç; Genel Kasa'da dahil TRON düzeltmesi uygulanır.
 */
export function sumOperatingKasaLedgerBalance(
  ...
  return kasas
    ...
    .reduce(
      (s, k) =>
        s + getKasaDisplayBalance(k, kasas, kasaTransactions, panel).balance,
      0,
    );
}

export type KasaOperatingMetrics = {
  operatingTotal: number;
  genelDisplayBalance: number | null;
};

/** TRON gizli oturumlar için sunucuda hesaplanan kasa özetleri. */
export function computeKasaOperatingMetrics(
  kasas: Kasa[],
  kasaTransactions: KasaTransaction[],
): KasaOperatingMetrics {
  const panel = computeTronPanelMetrics(kasas, kasaTransactions);
  const genel = panel?.genelKasa ?? findGenelKasa(kasas);
  return {
    operatingTotal: sumOperatingKasaLedgerBalance(kasas, kasaTransactions, panel),
    genelDisplayBalance: genel
      ? getKasaDisplayBalance(genel, kasas, kasaTransactions, panel).balance
      : null,
  };
}
